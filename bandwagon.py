...
# Convert the matrix to a NumPy array and normalize
user_rating_array = user_movie_matrix.to_numpy()
scaler = StandardScaler()
user_rating_vectors_normalized = scaler.fit_transform(user_rating_array)

print(user_movie_matrix)

# apply DBSCAN algorithm
user_rating_vectors_normalized[user_rating_vectors_normalized == 0] = np.nan
distacne_matrix = nan_euclidean_distances(user_rating_vectors_normalized, user_rating_vectors_normalized)
eps = 0.3
min_samples = 7

dbscan = DBSCAN(eps=eps, min_samples=min_samples, metric='precomputed')
labels = dbscan.fit_predict(distacne_matrix)
...
# displaying data after DBSCAN algorithm
